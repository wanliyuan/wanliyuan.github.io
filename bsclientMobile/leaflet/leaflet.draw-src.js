...
/*			var me = this;
			setTimeout(function(){
				me._markers[me._markers.length - 1].on('click', me._finishShape, me)
			}, 1000);*/

			var me = this;
			setTimeout(function(){
				if(me._markers && me._markers[me._markers.length - 1])
					me._markers[me._markers.length - 1].on('click', me._finishShape, me)
			}, 1000);
	...
	_onTouchEnd: function () {
		...
		if (this._touchOriginPoint) {
			...
			if (this._touchEndPoint) {
				...
				if (Math.abs(distanceMoved) < 9 * (window.devicePixelRatio || 1)) {
					this.addVertex(this._currentLatLng);
					this._touchPrevPoint = this._touchOriginPoint;
				}
			} else {
				var distancePixels = this._touchPrevPoint ? this._touchPrevPoint.distanceTo(this._touchOriginPoint) : 0;
				if(!this._touchPrevPoint || distancePixels > 15){
					this.addVertex(this._currentLatLng);
					this._touchPrevPoint = this._touchOriginPoint;
				}
			}
